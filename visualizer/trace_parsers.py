...
class _CodexRequestAccounting:
    """Track only server-reported request usage; active context is not public."""
    ...
    def add_token_count(self, info: JsonDict) -> int:
...
def parse_codex_jsonl(path: Path, _depth: int = 0) -> JsonDict:
    ...
    requests = _CodexRequestAccounting()
    ...
    for record in _iter_jsonl(path):
        ...
        if rtype == "session_meta":
            ...
            requests.mark_compacted()
        elif rtype == "event_msg":
            etype = payload.get("type")
            if etype == "token_count":
                info = _as_dict(payload.get("info"))
                request_tokens = requests.add_token_count(info)
